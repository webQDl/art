      ...
      <div>
        ...
        <div>
          <div style="margin-bottom:10px">
            ...
            <button v-show="!loginState" class="verificationCode">获取验证码</button>
          </div>
          <router-link :to="{path :'/forgetPwd'}"><div v-show="loginState" class="forget">忘记密码?</div></router-link>
        </div>
      </div>
      <div class="submitBtn"><van-button round block color="#0096C7" native-type="submit">登录</van-button></div>
...
export default {
  ...
  computed: {
    pwdPholder () {
      return this.loginState ? '请输入密码' : '请输入验证码'
    }
  },
  methods: {
    onClickLeft () {
